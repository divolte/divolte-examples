...
mapping {
    ...
    section {
       ...
       */
      def topLevelPageMatcher = match '^/static/javadoc/([^/\\.]+)\\.html$' against locationUri.path()
       ...
       */
      def packageSummaryMatcher = match '^/static/javadoc/(.+?)/package-summary.html$' against locationUri.path()
       ...
       */
      def packageTreeMatcher = match '^/static/javadoc/(.+)/package-tree\\.html$' against locationUri.path()
       ...
       */
      def classMatcher = match '^/static/javadoc/(.+)/(.+)\\.html$' against locationUri.path()
       ...
       * Search page
       */
      when locationUri.path().equalTo('/search/') apply {
        map 'search' onto 'pageType'
    ...
    /*
     * Is this a direct click from the search result page?
     */
